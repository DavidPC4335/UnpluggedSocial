...
#unplugged-popover-title{font-weight:600;margin-bottom:8px;font-size:22px;color:#000;}
#unplugged-popover-subtitle{color:#000;margin-bottom:8px;font-size:14px;}
...
export function install() {
  try {
    if ((window as any).ReactNativeWebView?.postMessage) {
      (window as any).ReactNativeWebView.postMessage('[UN] welcomePopover: install start');
    }
  } catch {}
  ...
  function show(force = false) {
    ...
    subtitle.id = 'unplugged-popover-subtitle';
    subtitle.textContent = 'Shake your phone to open the menu.';
